(function ($) {
    ...
    $(document).on("click", "body", function (e) {
        ...
        if (!$(e.target).closest(".navbar-nav").length) {
            ...
            $(e.target).parents("body").find(".header-cart.open").length
        ) {
            // Close cart
            $(".header-cart").trigger("mouseleave");
            $(".header-cart").removeClass("show");
    ...
    function setTopSpaceHeight() {
        if (
            (!$("header").hasClass("sticky") &&
                ($(".top-space-margin").length ||
                    $(".top-space-padding").length)) ||
            ...
            if (dropdownWidth > menuRightPosition) {
                if (element.find(".dropdown-menu").length > 0) {
                    var menuTopPosition = element.position().top,
                        ...
                        totalHeight =
                            menuTopPosition + submenuHeight + getHeaderHeight(),
    ...
    if (typeof $.fn.mCustomScrollbar === "function") {
        $("[data-scroll-options]").each(function () {
            var _this = $(this);
            var scrollOptions =
                _this.data("scroll-options") || '{"theme": "dark"}';
            ...
            if (typeof TweenLite !== "undefined") {
                document.addEventListener("mousemove", function (e) {
                    ...
                    TweenLite.to(_self.querySelector(".hover-reveal"), 0.6, {
                        x:
                            posX + imgWidth > windowWidth
                                ? e.clientX - imgWidth
                                : posX,
                        y:
                            posY + imgHeight > windowHeight
                                ? e.clientY - imgHeight
                                : posY,
                    });
                });
            ...
            initEvents() {
                ...
                this.DOM.menuLinks.forEach((item, i) => {
                    ...
                    item.addEventListener("mouseenter", (e) => {
                        item.closest(".menu").childNodes.forEach(
                            (item) =>
                                item.classList &&
    ...
    $dataAnimeElements.each(function () {
        ...
        if (animeOptions && getWindowWidth() > animeBreakPoint) {
            try {
                const effect =
                    animeOptions.effect && animeOptions.effect.toLowerCase();
    ...
    const slideAnimation = (target, options) => {
        ...
        slide_anime
            .add({
                targets: tmp,
                scaleX:
                    direction === "lr" || direction === "rl" ? [0, 1] : [1, 1],
                scaleY:
                    direction === "tb" || direction === "bt" ? [0, 1] : [1, 1],
                duration: duration + 500,
                easing: "easeInOutCubic",
                delay: delay,
                complete: function () {
                    if (direction === "lr") {
                        tmp.style.WebkitTransformOrigin =
                            tmp.style.transformOrigin = "100% 50%";
                    } else if (direction === "tb") {
                        tmp.style.WebkitTransformOrigin =
                            tmp.style.transformOrigin = "50% 100%";
                    } else if (direction === "bt") {
                        tmp.style.WebkitTransformOrigin =
                            tmp.style.transformOrigin = "0% 0%";
                    } else {
                        tmp.style.WebkitTransformOrigin =
                            tmp.style.transformOrigin = "0% 50%";
                    }
    ...
    const curvedTextAnimation = (target, options) => {
        ...
        curveText
            ...
            .add(
                {
                    targets: target.querySelectorAll(
                        ".anime-text > .word > .char"
                    ),
                    duration: 1000,
                    easing: "easeInExpo",
            ...
            .add(
                {
                    ...
                    opacity: [0, 1],
                    translateX:
                        options.direction === "left" ? [100, 0] : [-100, 0],
                },
                "-=600"
            );
    };
    ...
    const smoothWaveTextAnimation = (target, options) => {
        ...
        smoothWaveText
            ...
            .add({
                ...
                opacity: content.length > 1 ? [1, 0] : [1, 1],
                translateY:
                    content.length > 1 ? (direction === "down" ? 50 : -50) : 0,
                duration: 500,
                easing: "easeOutQuad",
                delay: anime.stagger(speed ? speed : 40, {
                    start: duration - 1000,
    ...
    const rubberbandTextAnimation = (target, options) => {
        ...
        rubberband
            ...
            .add({
                targets: target.querySelectorAll(".anime-text > .word > .char"),
                translateX:
                    content.length > 1 ? (direction === "left" ? -40 : 40) : 0,
                opacity: content.length > 1 ? 0 : 1,
                easing: "easeInExpo",
                duration: 500,
                delay: anime.stagger(speed ? speed : 75, {
    ...
    function FancyTextDefault(item, ftOptions) {
        ...
        if (content) {
            ...
            item.querySelector(".anime-text").setAttribute(
                "data-splitting",
                true
            );
            Splitting();
    ...
    if (typeof $.fn.magnificPopup === "function") {
        if ($("#subscribe-popup").length > 0) {
            let delaySecond = 1.5,
                expireDays = 30,
                cookieName = "open-eye-promo-popup";
    ...
    $(document).on("click", ".submit", function () {
        ...
        formObj.find(".required").each(function () {
            ...
            if (fieldVal == "" || fieldVal == undefined) {
                ...
                __this.attr("type") == "url" &&
                !urlformat.test(fieldVal)
            ) {
                error = true;
                __this.addClass("is-invalid");
            } else if (
                ...
                if (
                    ...
                    if ($(".ipad-top-space-margin").length) {
                        if (getWindowWidth() <= menuBreakPoint) {
                            $(
                                "section:first.full-screen, section:first .full-screen"
                            ).css("height", minHeight - headerHeight);
                        } else {
                            $(
                                "section:first.full-screen, section:first .full-screen"
                    ...
                    if (
                        $("header nav.navbar").hasClass("ipad-top-space-margin")
                    ) {
                        if (getWindowWidth() <= menuBreakPoint) {
                            minHeight =
                                minHeight -
                                $("header nav.navbar").outerHeight();
                        }
                    } else {
                        minHeight =
                            minHeight - $("header nav.navbar").outerHeight();
                    }
        ...
        swipers.forEach(function (swiperItem) {
            ...
            if (
                ...
                sliderOptions["on"] = {
                    init: function () {
                        ...
                        let activeIndex = this.activeIndex,
                            ...
                            anime_el =
                                current_slide.querySelectorAll("[data-anime]"),
                            fancy_el =
                                current_slide.querySelectorAll(
                                    "[data-fancy-text]"
                                );
                        ...
                        if (getWindowWidth() > animeBreakPoint) {
                            if (fancy_el) {
                                fancy_el.forEach((element) => {
                                    element.classList.add("appear");
                                    let fancy_options =
                                        element.getAttribute("data-fancy-text");
                                    if (
                                        typeof fancy_options !== "undefined" &&
                                        fancy_options !== null
                                    ) {
                                        fancy_options =
                                            $.parseJSON(fancy_options);
                                        let child = element;
                ...
                var isNumberPaginationProgress =
                    _this.attr("data-swiper-number-pagination-progress") ||
                    false;
                var showProgress =
                    _this.attr("data-swiper-show-progress") || false;
                ...
                if (getWindowWidth() > overlayBreakpoint) {
                    setTimeout(function () {
                        _this.imagesLoaded(function () {
                            ...
                            var parentSectionObj =
                                    closestSectionObj.prev(".overlap-height"),
    ...
    function addBoxAnimationClass(boxObj) {
        if (boxObj.length) {
            ...
            if (visible >= 0.5) {
                if (
                    typeof boxObj.attr("data-animation-delay") !==
                        "undefined" &&
    ...
    if (typeof particlesJS === "function" && particleItems.length) {
        $.each(particleItems, function (index, particleItem) {
            ...
            var particleItemOptions = $(particleItem).attr(
            ...
            if ($(window).width() > 1199) {
                ...
                if (stackItems.length) {
                    ...
                    for (let i = 0; i < stackItems.length - 1; i++) {
                        var stackBoxHeight =
                            (_this.height() /
                                _this.find(".stack-item").length) *
                            i;
                        var stackTopPosition = _this.offset().top;
                        if (
                            windowScrollTop - stackTopPosition >
                            stackBoxHeight
                        ) {
                            var yMove =
                                windowScrollTop -
        ...
        return function (e, opts) {
            ...
            for (i = opts.elements.length; i--; ) {
                var elm = opts.elements[i],
                    ...
                    inViewport =
                        middle > opts.zone[1] && middle < 100 - opts.zone[1];
    ...
    function onScroll() {
        if (window.sections.length > 0) {
            const section = window.sections
                ...
                .find(
                    (section) => section.rect.bottom >= window.innerHeight * 0.5
                );
        ...
        if (marker !== null && marker !== undefined) {
            ...
            if (marker.type.toLowerCase() === "html") {
                ...
                HTMLMarker.prototype.draw = function () {
                    ...
                    let position = overlayProjection.fromLatLngToDivPixel(
                        this.pos
                    );
                    let panes = this.getPanes();
